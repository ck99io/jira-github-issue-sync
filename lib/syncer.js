(function syncer() {
  ...
  var getGhUserFor = function getGhUserFor(jiraUser) {
    var ghuser;
    if(jiraUser == null){
        ghuser = context.config.userMapping["undefined"];
    } else {
        ghuser = context.config.userMapping[jiraUser.name];
    }
    return ghuser;
  };
  ...
  var createGhIssue = function createGhIssue(jiraIssue, callback) {
    context.api.jira.default.findIssue(jiraIssue.key, function getIssue(error, completeIssue) {
        ...
        var args = _.extend({
            assignee: getGhUserFor(completeIssue.fields.assignee),
