(function syncer() {
  ...
  var linkGhSubtasks = function linkGhSubtasks(subtask, ghIssueBody, callback){
    var parentId = getGhParentId(subtask.__parentId, function(parentId){
      console.log(parentId);
      return parentId;
    });

    console.log("Associating subtask", subtask.__parentId);
    if (!subtask.__parentId) {
      return callback(null);
    }

    var args = _.extend({
      body: "Related to #" + subtask.__parentId + " as sub-task"
    });
